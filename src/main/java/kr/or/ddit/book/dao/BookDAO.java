...
public class BookDAO {
	...
	public Map<String, Object> selectBook(Map<String, Object> map) {
		 ...
		 * 만약 쿼리 결과 행 수가 0개면 selectOne 메소드는 num을 반환하게 되고,
	...
	public int removeBook(Map<String, Object> map) {
		/*
		 * sqlSessionTemplate 객체의 deleteBook 메소드는 update 메소드와 사용법이 동일하다.
		 * 첫번째 파라미터는 쿼리 ID, 두번재 파라미터는 쿼리 파라미터이고 반환값은 영향받은 행 수이다.
		 */
	...
	public List<Map<String, Object>> selectBookList(Map<String, Object> map) {
		/*
		 * 쿼리 겨로가를 목록으로 받기 위해서는 sqlSessionTemplate.selectList를 사용할 수 있다.
		 * 첫뻔째 파라미터는 쿼리 ID, 두번째 파라미터는 쿼리 파라미터이다.
		 * 리턴타입을 List 타입으로 설정한 건, selectList 메소드의 결과가 집합 목록을 변환하기 때문이다.
		 */
