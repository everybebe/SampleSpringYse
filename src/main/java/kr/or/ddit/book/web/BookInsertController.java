...
/*
 * @Controller 어노테이션에 있는 클래스는 스프링이 브라우저의 요청(request)을 받아들이는 컨트롤러라고 인지해서
...
public class BookInsertController {
	 ...
	 * 의존성 주입을 통한 결합도 낮추기 
	 */
	@Inject //싱글톤 역할을 해줌 , 이미 만들어놓은 객체를 주입해서 쓰겠다는 어노테이션
	 ...
	 * 
	 * ModelAndView는 컨드롤러가 반환할 데이터를 담당하는 모델(Model)과 화면을 담당하는 뷰 (View)의 경로를 합쳐놓은  객체다.
	 ...
	 * viewresolver라는 녀석이 제일 먼저 받아 surffix, prefix 속성에 의해서 앞에는 '/WEB-INF/views/'가 붙고
	...
	public ModelAndView bookForm() {
		return new ModelAndView("book/form"); //forward 방식 . book안에 있는 form.jsp를 찾아줘. 뷰를 리턴
	...
	public ModelAndView insertBook(@RequestParam Map<String, Object> map) {
		...
		// 서비스 메소드 insertBook 호출하여 책을 등록한다.
